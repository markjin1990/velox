...
void HashTable<ignoreNullKeys>::parallelJoinBuild() {
  ...
  for (auto i = 0; i < numPartitions; ++i) {
    ...
    VELOX_CHECK_GE(buildPartitionBounds_[i], 0);
  }
...
// HashTable.
int32_t
findPartition(int64_t index, const int64_t* bounds, int32_t numPartitions) {
  // The partition bounds are padded to batch size.
  constexpr int32_t kBatch = xsimd::batch<int64_t>::size;
  auto indexVector = xsimd::batch<int64_t>::broadcast(index);
  for (auto i = 1; i < numPartitions; i += kBatch) {
    auto bits = simd::toBitMask(
        indexVector < xsimd::batch<int64_t>::load_unaligned(bounds + i));
    if (bits) {
...
void HashTable<ignoreNullKeys>::partitionRows(
  ...
  while (auto numRows = subtable.rows_->listRows(
    ...
    VELOX_DCHECK_EQ(
        0,
        buildPartitionBounds_.capacity() % xsimd::batch<int64_t>::size,
...
FOLLY_ALWAYS_INLINE void HashTable<ignoreNullKeys>::buildFullProbe(
  ...
  auto insertFn = [&](int32_t /*row*/, int64_t index) {
